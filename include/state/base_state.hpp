...
struct BaseState : public boost::msm::front::state<> {
    ...
    void on_entry(Event const&, FSM&) {
        std::cout << fmt::format("[{}]: 进入状态 {}", common::getCurrentTimestamp(), static_cast<Derived*>(this)->get_state_name()) << std::endl;
        // static_cast<Derived*>(this)->do_on_entry();
    }

    template <class Event, class FSM>
    void on_exit(Event const&, FSM&) {
        std::cout << fmt::format("[{}]: 离开状态 {}", common::getCurrentTimestamp(), static_cast<Derived*>(this)->get_state_name()) << std::endl;
